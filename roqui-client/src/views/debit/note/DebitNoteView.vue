<template>
  ...
  <section class="section" v-show="!isLoading">
    <div class="container">
      <h1 class="title">Notas de Débito</h1>
      ...
      <div class="field is-horizontal m-3">
        <div class="radios">
          <label class="radio">
            <input type="radio" value="All" v-model="status" @keyup.enter="findDebitNotes" />
            Todos
          </label>
          <label class="radio">
            <input
          ...
          <label class="radio">
            <input type="radio" value="Authorized" v-model="status" @keyup.enter="findDebitNotes" />
            Autorizados
          </label>
        </div>
      </div>
      ...
      <div class="field is-grouped m-3">
        <button class="button is-primary" @click="findDebitNotes">Buscar</button>
    ...
    <AppDetail v-bind:details="debitNote.details" />
  </section>
</template>
<script>
...
import AppDetail from './DebitNoteDetailView.vue'
...
export default {
  ...
  computed: {
    finderMessage() {
      if (this.debitNote.details && this.debitNote.details.length > 0) {
        return `${this.debitNote.details.length}`
      }
      return '0'
    },
  },
  ...
  methods: {
    findDebitNotes() {
      ...
      debitNoteService
        ...
        .then((response) => {
          this.debitNote.details = response.data.map((detail) => ({
            ...detail,
    ...
    authorizeAll() {
      ...
      debitNoteService
        ...
        .catch((error) => {
          console.error('Error to authorize all debit notes:', error)
        })
    },
    ...
    checkAll() {
      ...
      debitNoteService
        ...
        .catch((error) => {
          console.error('Error to check all debit notes:', error)
        })
    },

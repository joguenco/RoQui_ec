<template>
  ...
  <section class="section" v-show="!isLoading">
    <div class="container">
      <h1 class="title">Comprobantes de Retención</h1>
      ...
      <div class="field is-horizontal m-3">
        <div class="radios">
          <label class="radio">
            <input type="radio" value="All" v-model="status" @keyup.enter="findWithholds" />
            Todos
          </label>
          <label class="radio">
            <input
          ...
          <label class="radio">
            <input type="radio" value="Authorized" v-model="status" @keyup.enter="findWithholds" />
            Autorizados
          </label>
        </div>
      </div>
      ...
      <div class="field is-grouped m-3">
        <button class="button is-primary" @click="findWithholds">Buscar</button>
    ...
    <AppDetail v-bind:details="withhold.details" />
  </section>
</template>
<script>
import withholdService from '@/services/withhold-service'
import AppHeader from '@/components/layout/AppHeader.vue'
import AppLoader from '@/components/shared/AppLoader.vue'
import AppNotification from '@/components/shared/AppNotification.vue'
import AppDetail from './WithholdDetailView.vue'
...
export default {
  ...
  computed: {
    finderMessage() {
      if (this.withhold.details && this.withhold.details.length > 0) {
        return `${this.withhold.details.length}`
      }
      return '0'
    },
  },
  ...
  methods: {
    findWithholds() {
      ...
      withholdService
        ...
        .then((response) => {
          this.withhold.details = response.data.map((detail) => ({
            ...detail,
        ...
        .catch((error) => {
          console.error('Error fetching withhold:', error)
        })
    },
    ...
    authorizeAll() {
      ...
      withholdService
        ...
        .catch((error) => {
          console.error('Error to authorize all withholds:', error)
        })
    },
    ...
    checkAll() {
      ...
      withholdService
        ...
        .catch((error) => {
          console.error('Error to check all withholds:', error)
        })
    },

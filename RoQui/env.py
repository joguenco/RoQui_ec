...
def cmd_ps():
    ...
    print("    por limpiar ........... %s" % len(sucios))

    if not mal and not rotos and not sucios:
        print("    OK, todo consistente con %s" % motor)
    if sucios:
        print()
        print("  ESTOS HAY QUE LIMPIARLOS (linea comentada o import de UUID):")
...
def cmd_cambiar(destino):
    ...
    if actual == destino:
        ...
        pendiente = mal or rotos or sucios
        if not pendiente:
            print()
...
def cmd_check(destino):
    ...
    if sucios:
        # o les falta el import de UUID, o les sobra, o quedo el @Id comentado
        print("    %s hay que limpiarlos" % len(sucios))
    if mal:

...
public class WithholdDocumentTaxDto
{
    [Required]
    ...
    [Required]
    [RegularExpression(@"^\d{1,4}$", ErrorMessage = "PercentageCode must contain between 1 and 4 digits.")]
    public required string PercentageCode { get; set; }

    [Required]
    [Range(0, double.MaxValue, ErrorMessage = "TaxBase cannot be negative.")]
